...
### 错误 22：smartQuotes 替换行内代码内的引号

**做法**：弯引号正则 `/"([^"]*)"/g`、`/'([^']*)'/g` 在 HTML 标签占位保护后执行。

**问题**：`` `"hello"` `` 等行内代码中的引号被替换为弯引号，破坏了代码内容。此前已保护代码块（```` ``` ```` 围栏，占位符 `\u0000CODE{n}\u0000`），但行内代码（单个/多个反引号包裹）未在保护范围内。

**解决**：smartQuotes 开头先保护行内代码——`/(`+)([^`\n]*?)\1/g` 匹配配对反引号（支持 `` `` ` ` `` 包裹含反引号内容），替换为 `\u0000CODEINLINE{n}\u0000` 占位符，引号替换完成后还原。占位符前缀与代码块 `\u0000CODE` 区分，互不冲突；行内代码禁止跨行（排除 `\n`）。

**验证**：`` `"hello"` ``、`` 'it\'s' `` 等行内代码内引号保持原样；正文 `"hello"` 仍正常替换为弯引号。

**教训**：占位保护要覆盖所有结构化内容层级。代码块与行内代码是两种独立标记，围栏占位保护无法覆盖行内代码，需在规则内部（smartQuotes）单独保护。

---

### 错误 23：cjkSpacing 等规则仍破坏行内代码——改为全局占位保护

**做法**：错误 22 在 smartQuotes 内部保护行内代码。

**问题**：行内代码仍受其他规则影响——`中文\`code 中文\`` 中的中英文空格（cjkSpacing）会在代码内插入空格，`\`a b\`` 被 removeMultipleSpaces 合并空格等。规则级保护只能覆盖单一规则，无法覆盖全部 27 条规则。

**解决**：将行内代码保护提升到 formatMarkdown 全局占位层（与 `<span>`、代码块同级）——在代码块保护之后用 `/(`+)([^`\n]*?)\1/g` 提取行内代码为 `\u0000CODEINLINE{n}\u0000` 占位符，格式化流水线结束后还原。同时移除 smartQuotes 内部的行内代码保护（已冗余）。还原顺序遵循"外层先还原"：SPAN → CODEINLINE → CODE。

**验证**：行内代码内引号、中英文空格、多余空格等均保持原样；正文格式化不受影响。

**教训**：内容保护应放在全局占位层统一处理，而非分散到各规则内部。所有规则共享同一份已占位文本，规则内部无需也无法感知行内代码。
